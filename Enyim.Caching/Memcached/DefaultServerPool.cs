...
namespace Enyim.Caching.Memcached
{
	public class DefaultServerPool : IServerPool, IDisposable
	{
		...
		{
            var isDebug = _logger.IsDebugEnabled;

			if (isDebug) _logger.Debug("Checking the dead servers.");
			...
			lock (this.DeadSync)
			{
				if (this.isDisposed)
				{
					if (_logger.IsWarnEnabled)
                        _logger.Warn("IsAlive timer was triggered but the pool is already disposed. Ignoring.");
				...
				for (var i = 0; i < nodes.Length; i++)
				{
					...
					if (n.IsAlive)
					{
						if (isDebug) _logger.DebugFormat("Alive: {0}", n.EndPoint);

						aliveList.Add(n);
					}
					else
					{
						if (isDebug) _logger.DebugFormat("Dead: {0}", n.EndPoint);
						...
						if (n.Ping())
						{
							...
							if (isDebug) _logger.Debug("Ping ok.");
						}
						else
						{
							if (isDebug) _logger.Debug("Still dead.");
				...
				if (changed)
				{
					if (isDebug) _logger.Debug("Reinitializing the locator.");
				...
				if (deadCount == 0)
				{
					if (isDebug) _logger.Debug("deadCount == 0, stopping the timer.");

					this.isTimerActive = false;
				}
				else
				{
					if (isDebug) _logger.DebugFormat("deadCount == {0}, starting the timer.", deadCount);
		...
		{
            var isDebug = _logger.IsDebugEnabled;
			if (isDebug) _logger.DebugFormat("Node {0} is dead.", node.EndPoint);
			...
			lock (this.DeadSync)
			{
				if (this.isDisposed)
				{
					if (_logger.IsWarnEnabled) _logger.Warn("Got a node fail but the pool is already disposed. Ignoring.");
				...
				{
					if (isDebug) _logger.Debug("Starting the recovery timer.");
					...
					if (isDebug) _logger.Debug("Timer started.");
				}
			}
		}
		...
		void IDisposable.Dispose()
		{
			...
			lock (this.DeadSync)
			{
				...
				if (nd != null)
					try { nd.Dispose(); }
					catch (Exception e) { _logger.Error(nameof(DefaultServerPool), e); }
